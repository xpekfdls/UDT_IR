		...
		if (SUCCEEDED(hr))
		{
			...
			if (m_bStart)
			{
				...
				else
				{
					...
					if (fSpeed_p < m_fFast && m_switch == 0)
						m_fSpeed = fSpeed_p + 0.075f / m_fFPS;
					else if (fSpeed_p >= m_fFast&& m_switch == 0)
					{
						m_fSpeed = m_fFast;
					}
					...
					if (fSpeed_p >= m_fFast && m_switch == 0 && m_fFast != 0)
					{
							m_fStartTime = m_fTime;
							m_switch = 1;
					}
					else if (m_switch == 2 && fSpeed_p == m_fFast)
					{
						m_fStartTime = m_fTime;
						m_switch = 1;
					}
					else if (fSpeed_p == m_fSlow && m_switch == 1)
					{
						m_fStartTime = m_fTime;
						m_switch = 2;
						m_count++;
					...
					//for stop sequence
					if (m_count == 3)		//when m_count = 1, after run the last process stop the treadmill
						m_switch = 3;
					...
					if (m_switch == 2)
					{
						if (m_fDiffTime >= 20 && m_fDiffTime < 25)
						{
							if (fSpeed_p < m_fFast)
								m_fSpeed = fSpeed_p + 0.075f / m_fFPS;
							else
				...
				if (m_hAOTask)
				{
					//float64 data[1] = { -m_fSpeed / 0.0505f * 60 / 28 / 3.141592f * 30 / 100 + 0.1};
					float64 data[1] = { m_fSpeed / 0.0475f * 25 / 7 * 30 / 3.141592f / 100 };
				...
				if (m_pVRClientSocket)
				{
					...
					sprintf_s(strSend2, "%.3f %.2f %d\n", m_fSpeed, m_fHeadAngle /*,CalcArm(Joints)*/);
					m_pVRClientSocket->Send(strSend2, static_cast<int>(strlen(strSend2)));
				}
...
void CUDT_IRDlg::ProcessClose()
{
	...
	if (m_hAOTask)
	{
		while (m_fSpeed > 0)
		{
			...
			float64 data[1] = { -m_fSpeed / 0.0505f / 2 / 3.141592f * 60 * 60 / 28 / 150 };
			if (DAQmxFailed(DAQmxWriteAnalogF64(m_hAOTask, 1, TRUE, 10.0, DAQmx_Val_GroupByChannel, data, NULL, NULL)))
